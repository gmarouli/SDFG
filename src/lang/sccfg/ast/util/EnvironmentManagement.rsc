...
Environment mergeEnvironmentsInBlock(environment(env1, contEnv1, brEnv1, retEnv1), environment(env2, contEnv2, brEnv2, retEnv2)){
	env = mergeInBlockEnvironments(env1, env2);
	contEnv = mergeInBlockEnvironments(contEnv1, contEnv2);
	brEnv = mergeInBlockEnvironments(brEnv1, brEnv2);
	retEnv = mergeInBlockEnvironments(retEnv1, retEnv2);
	return environment(env, contEnv, brEnv, retEnv);
...
Environment updateEnvironments(environment(env1, contEnv1, brEnv1, retEnv1), environment(env2, _, _, retEnv2))
	= environment(updateEnvironments(env1, env2), contEnv1, brEnv1, mergeInBlockEnvironments(retEnv1, retEnv2));

map[loc,set[loc]] updateEnvironments(map[loc,set[loc]] env, map[loc,set[loc]] tempEnv){

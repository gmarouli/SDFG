...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:arrayAccess(ar, index), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:newArray(_, dimensions, init), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	potential1 = {};
	for(d <- dimensions){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:newArray(t, dimensions), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:arrayInitializer(list[Expression] elements), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	potential = {};
	for(el <- elements){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:assignment(lhs,_,rhs), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	...
	env = setVariableDependencies(env, var, {e@src});
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:cast(_, exp), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:newObject(expr, _, args, _), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:newObject(expr, _, args), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	return gatherStmtFromExpressions(m, Expression::newObject(expr, args)[@src = e@src][@decl = e@decl], env, exs, stmts);
}

//newObject(Expression expr, list[Expression] args, Declaration class)
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:newObject(expr, args, _), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:newObject(expr, args), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	potential = {};
	for(arg <- args){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:conditional(cond,ifB,elseB), Environment env, map[loc, Environment] exs, set[Stmt] stmts){	
	<stmts, potential, env, exs> = gatherStmtFromExpressions(m, cond, env, exs, stmts);
	stmts += potential;
	return branching(m, \expressionStatement(ifB), \expressionStatement(elseB), env, exs, stmts);
}

//fieldAccess(bool isSuper, Expression expression, str name)
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:fieldAccess(_,exp,_), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	<stmts, potential, env, exs> = gatherStmtFromExpressions(m, exp, env, exs, stmts);
	potential = {Stmt::read(e@src, e@decl, writtenBy) | writtenBy <- getVariableDependencies(env,e@decl)} + {Stmt::read(e@src, e@decl, dependOn) | dependOn <- getDependencyIds(potential)};	
	return <stmts, potential, env, exs>;
}

//fieldAccess(bool isSuper, str name)
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:fieldAccess(_, _), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	potential = {Stmt::read(e@src, e@decl, writtenBy) | writtenBy <- getVariableDependencies(env,e@decl)};	
	return <stmts, potential, env, exs>;
}

//instanceof(Expression leftside, Type rightSide)
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:\instanceof(lsh,_), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:methodCall(isSuper,name,args), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:methodCall(isSuper, receiver, name, args), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	for(arg <- args){
		<stmts, potential, env, exs> = gatherStmtFromExpressions(m, arg, env, exs, stmts);
		stmts += potential;
	}
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:variable(name,_,rhs), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	...
	env = setVariableDependencies(env, e@decl, {e@src});
	return <stmts, {}, env, exs>;
}

//variable(str name, int extraDimensions)
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:variable(name,_), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	return <stmts, {}, env, exs>;
}

//bracket(Expression exp);
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:\bracket(exp), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	return gatherStmtFromExpressions(m, exp, env, exs, stmts);
}

//this()
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:this(), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:declarationExpression(d), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
...
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e:postfix(operand, operator), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	if(operator == "++" || operator == "--"){
		<stmts, potenial, env, exs> = gatherStmtFromExpressions(m, operand, env, exs, stmts);
		stmts += potential;
		stmts += {Stmt::assign(e@src, operand@decl, id) | id <- getDependencyIds(potential)};
	
		potential = {Stmt::read(operand@src, operand@decl, writtenBy) | writtenBy <- getVariableDependencies(env, operand@decl)};				
		env = setVariableDependencies(env, operand@decl, {e@src});
	
		return <currentBlock, potential, env, exs>;
	}
	else{
...
//simpleName(str name)
tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m , Expression e:simpleName(name), Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	potential = {Stmt::read(e@src, e@decl, writtenBy) | writtenBy <- getVariableDependencies(env,e@decl)};	
	return <stmts, potential, env, exs>;
}

default tuple[set[Stmt], set[Stmt], Environment, map[loc, Environment]] gatherStmtFromExpressions(Declaration m, Expression e, Environment env, map[loc, Environment] exs, set[Stmt] stmts){
	return <stmts, {}, env, exs>;
}
